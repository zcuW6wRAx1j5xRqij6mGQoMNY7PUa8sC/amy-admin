...
import { SpotWithdrawAudit } from '@/service/api/order';
import { isEmpty } from '@/utils/is';

defineOptions({
  name: 'SpotWithdrawAuditDrawer'
});
...
const title = computed(() => {
  ...
  return titles[props.auditType || 1];
});
...
async function handleSubmit() {
  ...
  try {
    await SpotWithdrawAudit({
      id: props.withdrawId,
      audit_status: props.auditType,
      remark: ruleForm.value.remark
    });

    const successMsg = props.auditType === 1 ? '审核通过成功' : '审核驳回成功';
    window.$message?.success(successMsg);
    closeDrawer();
    emit('submitted');
  } catch (error: any) {
    const errorMsg = props.auditType === 1 ? '审核通过失败' : '审核驳回失败';
    if (error?.message) {
