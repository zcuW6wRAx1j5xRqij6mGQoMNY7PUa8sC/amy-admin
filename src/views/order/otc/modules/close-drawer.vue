...
import { OrderOtcClose, type OtcCloseRequest } from '@/service/api/order';
...
const formData = ref<OtcCloseRequest>({
  id: 0,
  close_price: 0
});

function createDefaultModel(): OtcCloseRequest {
  return {
    id: 0,
    close_price: 0
  };
...
async function handleInitModel() {
  ...
  if (props.rowData) {
    formData.value = {
      id: props.rowData.id,
      close_price: props.rowData.match_price || 0
    };
  }
}
...
async function handleSubmit() {
  ...
  // 表单验证
  if (isEmpty(formData.value.close_price) || formData.value.close_price < 0) {
    errorObj.value.close_price = '请输入有效的平仓价格';
  }
  ...
  try {
    await OrderOtcClose(formData.value);
